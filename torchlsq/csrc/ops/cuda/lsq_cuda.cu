                                          ...
                                          const bool init_mode)
{
    ...
    TORCH_CHECK(shift.is_cuda(), "`shift` tensor must be CUDA tensor");
                                                       ...
                                                       const bool init_mode)
{
    ...
    auto iterS = torch::TensorIteratorConfig()
                 ...
                 .add_input(x)
                 .build();
    auto iterB = torch::TensorIteratorConfig()
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad.scalar_type(),
        ...
        [&] {
            ...
            at::native::gpu_kernel(iterX, [=] GPU_LAMBDA (scalar_t grad, scalar_t x) -> scalar_t {
                return lsq_backward_kernel_per_tensor_dX<scalar_t>(grad, x, s, inv_s, b, qmin, qmax, tmin, tmax, init_mode);
            });

            at::native::gpu_kernel(iterS, [=] GPU_LAMBDA (scalar_t grad, scalar_t x) -> scalar_t {
                return eval_mode?zero:lsq_backward_kernel_per_tensor_dS<scalar_t>(grad, x, s, inv_s, b, qmin, qmax, tmin, tmax, _grad_scaler, init_mode);
            });

            at::native::gpu_kernel(iterB, [=] GPU_LAMBDA (scalar_t grad, scalar_t x) -> scalar_t {
                return eval_mode?zero:lsq_backward_kernel_per_tensor_dB<scalar_t>(grad, x, s, inv_s, b, qmin, qmax, tmin, tmax, _grad_scaler, sym, init_mode);
            });
    });
                                           ...
                                           const bool init_mode)
{
    ...
    TORCH_CHECK(shift.dim() == 1, "shift should be a 1-D tensor");
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(x.scalar_type(),
        ...
        [&] {
            ...
            at::native::gpu_kernel(iter, [=] GPU_LAMBDA (scalar_t x, scalar_t s, scalar_t b) -> scalar_t {
                                                        ...
                                                        const bool init_mode)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad.scalar_type(),
        ...
        [&] {
            ...
            at::native::gpu_kernel(iterX, [=] GPU_LAMBDA (scalar_t grad, scalar_t x, scalar_t s, scalar_t b) -> scalar_t {
                return lsq_backward_kernel_per_channel_dX<scalar_t>(grad, x, s, b, qmin, qmax, tmin, tmax, init_mode, eps);
            });

            at::native::gpu_kernel(iterS, [=] GPU_LAMBDA (scalar_t grad, scalar_t x, scalar_t s, scalar_t b) -> scalar_t {
                return lsq_backward_kernel_per_channel_dS<scalar_t>(grad, x, s, b, qmin, qmax, tmin, tmax, _grad_scaler, init_mode, eps);
            });

            at::native::gpu_kernel(iterB, [=] GPU_LAMBDA (scalar_t grad, scalar_t x, scalar_t s, scalar_t b) -> scalar_t {
                return lsq_backward_kernel_per_channel_dB<scalar_t>(grad, x, s, b, qmin, qmax, tmin, tmax, _grad_scaler, sym, init_mode, eps);
            });
    });
